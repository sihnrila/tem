(() => {
  ...
  const signatureMotionImages = [
    'https://images.unsplash.com/photo-1509631179647-0177331693ae?auto=format&fit=crop&w=1200&q=80',
    'https://images.unsplash.com/photo-1558171813-4c088753af8f?auto=format&fit=crop&w=900&q=70',
    'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&w=900&q=70',
    'https://images.unsplash.com/photo-1496747611176-843222e1e57c?auto=format&fit=crop&w=900&q=70',
    'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=900&q=70',
    'https://images.unsplash.com/photo-1485968579580-b6d095142e6e?auto=format&fit=crop&w=900&q=70',
  ...
  brandContent['signature-motion-01'] = {
    logo: 'SIGNATURE MOTION',
    heroCopy: ['Campaign', 'Season 2025'],
    statement: 'Dynamic. Refined. Controlled energy.',
    products: [
      { name: 'Campaign Coat', price: 289000 },
      { name: 'Motion Jacket', price: 239000 },
      { name: 'Velocity Knit', price: 169000 },
      { name: 'Drift Trousers', price: 189000 },
      { name: 'Pulse Shirt', price: 149000 },
      { name: 'Arc Dress', price: 219000 },
      { name: 'Frame Top', price: 139000 },
      { name: 'Flow Wide Pants', price: 199000 }
    ],
    pdpName: 'Campaign Coat',
    pdpPrice: '₩289,000',
    pdpDesc: 'Full silhouette, controlled volume. A statement piece built for presence.',
    options: ['Charcoal', 'Off White', 'Deep Navy']
  };
  brandImages['signature-motion-01'] = signatureMotionImages;

  brandContent['signature-typo-01'] = {
    logo: 'SIGNATURE TYPO',
    heroCopy: ['Structured Silence', ''],
    statement: 'Structure is a form of clarity.',
    products: [
      { name: 'Refined Overcoat', price: 298000 },
      { name: 'Silence Shirt', price: 159000 },
      { name: 'Proportion Knit', price: 179000 },
      { name: 'Calm Trousers', price: 199000 },
      { name: 'Clarity Dress', price: 249000 },
      { name: 'Weight Jacket', price: 269000 },
      { name: 'Balance Top', price: 149000 },
      { name: 'Rest Wide Pants', price: 189000 }
    ],
    pdpName: 'Refined Overcoat',
    pdpPrice: '₩298,000',
    pdpDesc: 'Ultra refined construction. Cotton 100%, Italian mill. Relaxed oversized silhouette.',
    options: ['Cream', 'Sand', 'Soft Gray']
  };
  brandImages['signature-typo-01'] = signatureTypoImages;
  ...
  /* ===== Signature Motion — Section Content ===== */
  if (variantId === 'signature-motion-01') {
    setImageIfEmpty('.sig-hero-img', images[0], 'Campaign Hero');
    document.querySelectorAll('.sig-story-img').forEach((img, idx) => {
      if (!img.getAttribute('src')) {
        img.setAttribute('src', images[(idx + 1) % images.length]);
        img.setAttribute('alt', 'Story ' + (idx + 1));
      }
    });
    setImageIfEmpty('.sig-split-img', images[4], 'Split');
    setImageIfEmpty('.sig-campaign-img', images[5], 'Campaign');

    /* PDP */
    setImageIfEmpty('.sig-pdp-banner-img', images[0], 'Campaign Banner');
    setImageIfEmpty('.sig-pdp-product-img', images[1], brand.pdpName);
    document.querySelectorAll('.sig-pdp-sub-img').forEach((img, idx) => {
      if (!img.getAttribute('src')) {
        img.setAttribute('src', images[(idx + 2) % images.length]);
        img.setAttribute('alt', 'Detail ' + (idx + 1));
      }
    });
    const sigPdpName = document.querySelector('.sig-pdp-name');
    if (sigPdpName && !sigPdpName.textContent.trim()) sigPdpName.textContent = brand.pdpName;
    const sigPdpPrice = document.querySelector('.sig-pdp-price');
    if (sigPdpPrice && !sigPdpPrice.textContent.trim()) sigPdpPrice.textContent = brand.pdpPrice;
  }

  /* ===== Signature Typo — Section Content ===== */
  if (variantId === 'signature-typo-01') {
    setImageIfEmpty('.typo-split-img', images[0], 'Minimal');

    /* PDP */
    setImageIfEmpty('.typo-pdp-product-img', images[1], brand.pdpName);
    const typoPdpTitle = document.querySelector('.typo-pdp-hero-title');
    if (typoPdpTitle && !typoPdpTitle.textContent.trim()) typoPdpTitle.textContent = brand.pdpName;
    const typoPdpPrice = document.querySelector('.typo-pdp-price');
    if (typoPdpPrice && !typoPdpPrice.textContent.trim()) typoPdpPrice.textContent = brand.pdpPrice;
  }
